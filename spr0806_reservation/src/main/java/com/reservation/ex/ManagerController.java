...
public class ManagerController {
	...
	public String delete(HttpSession session, Model model,@RequestParam("email")String uEmail,RedirectAttributes rttr) throws Exception {
		System.out.println("ManagerController - /manager/manageUsers");
	...
	public String edit(HttpSession session, UserDto dto, Model model)
			throws Exception {
		System.out.println("ManagerController - /manager/selectUser");
	...
	public void selectContent(HttpSession session, HttpServletRequest request, HttpServletResponse response)
		...
		switch (action) {
		case "editProfile":
			content = "<div id='editProfileContent'>"
				    + "<h3>유저 정보 수정</h3>"
				    + "<form id='editProfileForm' action='/ex/manager/edit'>"
				    + "<div class='form-group'>"
				    + "<label for='name'>사용자 이름:</label>"
				    + "<input type='text' id='name' name='name' value='" + userDto.getName() + "' required>"
				    + "</div>"
				    + "<div class='form-group'>"
				    + "<label for='email'>이메일:</label>"
				    + "<input type='text' id='email' name='email' value='" + userDto.getEmail() + "' readonly>"
				    + "</div>"
				    + "<div class='form-group'>"
				    + "<label for='phone'>전화번호:</label>"
				    + "<input type='text' id='phone' name='phone' value='" + userDto.getPhone() + "' required>"
				    + "</div>"
		...
		case "deleteUser":
			content = "<div id='deleteUserContent'>"
		            + "<h3>유저 탈퇴</h3>"
		            ...
		            + "<button type='submit'>유저 탈퇴</button>"
		            + "</form>"
